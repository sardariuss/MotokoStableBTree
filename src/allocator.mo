...
module {
  ...
  public func loadAllocator(memory: Memory, addr: Address) : Allocator {
    ...
    let header = {
      ...
      _alignment           =                         memory.load(addr + 3 + 1,                 4);
      allocation_size      = Conversion.bytesToNat64(memory.load(addr + 3 + 1 + 4,             8));
      num_allocated_chunks = Conversion.bytesToNat64(memory.load(addr + 3 + 1 + 4 + 8,         8));
      free_list_head       = Conversion.bytesToNat64(memory.load(addr + 3 + 1 + 4 + 8 + 8,     8));
      _buffer              =                         memory.load(addr + 3 + 1 + 4 + 8 + 8 + 8, 16);
    };
  ...
  public class Allocator(variables: AllocatorVariables) {
    ...
    public func saveAllocator() {
      ...
      memory_.store(addr,                                                                 header.magic);
      memory_.store(addr + 3,                                                         [header.version]);
      memory_.store(addr + 3 + 1,                                                    header._alignment);
      memory_.store(addr + 3 + 1 + 4,                  Conversion.nat64ToBytes(header.allocation_size));
      memory_.store(addr + 3 + 1 + 4 + 8,         Conversion.nat64ToBytes(header.num_allocated_chunks));
      memory_.store(addr + 3 + 1 + 4 + 8 + 8,           Conversion.nat64ToBytes(header.free_list_head));
      memory_.store(addr + 3 + 1 + 4 + 8 + 8 + 8,                                       header._buffer);
    };
  ...
  func saveChunkHeader(header: ChunkHeader, addr: Address, memory: Memory) {
    ...
    memory.store(addr + 3 + 1,          Conversion.boolToBytes(header.allocated));
    memory.store(addr + 3 + 1 + 1,                             header._alignment);
  ...
  public func loadChunkHeader(addr: Address, memory: Memory) : ChunkHeader {
    let header = {
      magic =                            memory.load(addr,                 3);
      version =                          memory.load(addr + 3,             1)[0];
      allocated = Conversion.bytesToBool(memory.load(addr + 3 + 1,         1));
      _alignment =                       memory.load(addr + 3 + 1 + 1,     3);
      next =     Conversion.bytesToNat64(memory.load(addr + 3 + 1 + 1 + 3, 8));
    };
