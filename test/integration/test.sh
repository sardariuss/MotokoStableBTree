...
function install(wasm, args) {
  import interface = "2vxsx-fae" as ".dfx/local/canisters/test/test.did";
...
// Create a BTree
let btree_canister = install(file(".dfx/local/canisters/test/test.wasm"), args);
// Verify it is empty
...
upgrade(btree_canister, file(".dfx/local/canisters/test/test.wasm"), args);
call btree_canister.getLength();
assert _ == (1 : nat64);
call btree_canister.get(12345);
assert _ == opt("hello" : text);
...
reinstall(btree_canister, file(".dfx/local/canisters/test/test.wasm"), args);
call btree_canister.getLength();
assert _ == (0 : nat64);
call btree_canister.get(12345);
